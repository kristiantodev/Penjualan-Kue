...
class Laporan extends CI_Controller
{
    ...
    public function data_stok_exp_bulanan()
    {
        //cek login
        $this->is_login();
        ...
        $getData = $this->m_laporan->getDataExpStokBulanan($this->convert_bulan($bulan), $tahun);

        $data = [
            'title' => 'Laporan Bulanan Stok Expired',
            'bulan' => $bulan,
            'tahun' => $tahun,
            'data' => $getData
        ];

        $this->template->kasir('laporan/stok_exp_bulanan', $data);
    }

    public function cetak_stok_exp_bulanan($date)
    {
        $this->is_login();
        //explode url
        $exp = explode('-', $date);
        ...
        $getData = $this->m_laporan->getDataExpStokBulanan($this->convert_bulan($exp[0]), $exp[1]);
        ...
        $this->template->cetak('cetak/stok_exp_bulanan', $data);
    }

    public function data_stok_exp_tahunan()
    {
        //cek login
        $this->is_login();
        ...
        if ($this->input->post('cari', TRUE) == 'Search') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                redirect('stok_exp_tahunan');
            }
        } else {
            $tahun = date('Y');
        }

        $getData = $this->m_laporan->getDataExpStokTahunan($tahun);

        $data = [
            'title' => 'Laporan Tahunan Stok Expired',
            'tahun' => $tahun,
            'data' => $getData
        ];

        $this->template->kasir('laporan/stok_exp_tahunan', $data);
    }

    public function cetak_stok_exp_tahunan($tahun)
    {
        $this->is_login();
        ...
        $this->template->cetak('cetak/stok_exp_tahunan', $data);
    }
