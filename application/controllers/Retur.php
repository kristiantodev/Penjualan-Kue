        ...
        if ($this->input->post('submit', TRUE) == 'Submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                foreach ($this->cart->contents() as $c) {
                    ...
                    $dataExp = $exp->format('Y-m-d');
                    $this->db->query("UPDATE tbl_barang SET stok = 0 WHERE kode_barang='$kodeB'");
                    $this->db->query("UPDATE tbl_barang SET tgl_exp = '$dataExp' WHERE kode_barang='$kodeB'");
                    //push ke array cart
    ...
    public function edit_retur($id = null)
    {
        ...
        if ($this->input->post('submit', TRUE) == 'Update') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                //simpan data retur
                $update = $this->m_retur->update('tbl_retur', $data_retur, ['id_retur' => $idP]);

                if ($update) {
                    //hapus detail retur
                    $this->m_retur->delete('tbl_detail_retur', ['id_retur' => $idP]);
                    //simpan data detail retur
                    $this->m_retur->multiSave('tbl_detail_retur', $cart);
                    //kosongkan cart
                    $this->cart->destroy();
