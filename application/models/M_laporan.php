...
class M_laporan extends CI_Model
{
    ...
    function getDataExpStokBulanan($bulan, $tahun)
    {
		
        $tanggal1 = $tahun . '-' . $bulan . '-01';
        $tanggal2 = $tahun . '-' . $bulan . '-31';
        $table = 'barang_exp b
					WHERE tgl_exp >= \'' . $tanggal1 . '\'
					AND tgl_exp <= \'' . $tanggal2 . '\'';

        $select = 'kode_barang, nama_barang, brand, kemasan, stok, tgl_exp, stts_exp';

        $group = ['kode_barang', 'nama_barang', 'brand', 'kemasan', 'stok'];
    ...
    function getDataExpStokTahunan($tahun)
    {
        $table = 'barang b
					WHERE YEAR(tgl_masuk) = \'' . $tahun . '\'';

        $select = 'kode_barang, nama_barang, brand, kemasan, stok, stts_exp';
    ...
    function getDataStokHarian($tanggal)
    {
        ...
        $select = 'kode_barang, nama_barang, brand, kemasan, stok, SUM(c.qty) AS qty_penjualan, SUM(d.qty) AS qty_pembelian, SUM(e.qty) AS qty_penjualan_new, SUM(f.qty) AS qty_pembelian_new';
    ...
    {
        $select = 'p.id_pembelian AS id_pembelian, nama_barang, brand, kemasan, dp.harga AS harga, qty, nama_supplier, (SELECT COUNT(*) FROM tbl_detail_pembelian WHERE id_pembelian = p.id_pembelian) AS row';
    ...
    {
        $select = 'p.id_penjualan AS id_penjualan, nama_barang, brand, kemasan, dp.harga AS harga, qty, nama_pembeli, (SELECT COUNT(*) FROM tbl_detail_penjualan WHERE id_penjualan = p.id_penjualan) AS row';
    ...
    function getDataPenjualanBulanan($bulan, $tahun)
    {
        ...
        $select = 'p.id_penjualan AS id_penjualan, nama_barang, brand,  kemasan, dp.harga AS harga, qty, nama_pembeli, (SELECT COUNT(*) FROM tbl_detail_penjualan WHERE id_penjualan = p.id_penjualan) AS row_penjualan, (SELECT COUNT(*) FROM tbl_penjualan JOIN tbl_detail_penjualan dp ON(tbl_penjualan.id_penjualan = dp.id_penjualan) WHERE tgl_penjualan = p.tgl_penjualan) AS row_tanggal, tgl_penjualan';
